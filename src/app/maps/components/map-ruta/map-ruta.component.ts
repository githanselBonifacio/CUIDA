...
export class MapRutaComponent implements AfterViewInit{
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public citasProfesional: Tarea [],
  ) {}
  ...
  ngAfterViewInit(): void {
    let map = new mapboxgl.Map({
      ...
      center: [ this.citasProfesional[0].longitud,this.citasProfesional[0].latitud], // starting position [lng, lat]
      zoom: 15, 
      
    });
    let limites = new mapboxgl.LngLatBounds()
    for (let cita of this.citasProfesional){
      let marker = new mapboxgl.Marker({
        color:'#0033A0'
      })
      .setLngLat([cita.longitud, cita.latitud])
      .addTo(map);
    }
